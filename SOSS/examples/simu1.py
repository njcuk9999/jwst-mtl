...
comb, peaks = box_kim.make_comb(wave, peak_spacing=0.01, peak_width=0.001)
...
if True:
    imagelist = soss.generate_traces(WORKING_DIR + 'tmp/clear',   #'tmp/oversampling_{}/clear'.format(simuPars.noversample)
    ...
    imagelist = glob.glob(WORKING_DIR + 'tmp/clear*.fits')   #'tmp/oversampling_{}/clear*.fits'.format(simuPars.noversample)
    #imagelist = os.listdir(SIMUDIR)
    for i in range(np.size(imagelist)):
        imagelist[i] = os.path.join(SIMUDIR, imagelist[i])  #'oversampling_{}/'.format(simuPars.noversample)
    print(imagelist)
...
detector.add_noise(os.path.join(WORKING_DIR, 'test_clear.fits'),   #+ 'oversampling_{}/'.format(simuPars.noversample)
