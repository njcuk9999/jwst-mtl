...
class InputData:
    ...
    def photospectra(self):
        ...
        ###DL### int_time = (self.n_group - 1) * self.t_group / (60 * 60 * 24)
        # loop around order
        ...
        # add itime
        self.phot['ITIME'] = np.full_like(self.phot['WAVELENGTH'], self.time_int) ###DL###
...
# =============================================================================
# new class to import data produced by Michael Radica
# =============================================================================
class InputData1(InputData):
    def __init__(self, params, filename, verbose= False):
        # set filename
        self.filename = filename
        # ---------------------------------------------------------------------
        # print progress
        ...
        # Number of integrations
        self.n_int = data.extra_fits.Time.data.size ###data.meta.exposure.nints
        ...
        for onum in self.orders:
            self.spec[onum]={}
            key='Flux O'+str(onum)
            self.spec[onum]['FLUX']=data.extra_fits.instance[key]['data']   
            key='Flux Err O'+str(onum)
            self.spec[onum]['FLUX_ERROR']=data.extra_fits.instance[key]['data']   
            key1='Wave Low O'+str(onum)
            key2='Wave Up O'+str(onum)            
            self.spec[onum]['WAVELENGTH']=(data.extra_fits.instance[key1]['data']+
                                           data.extra_fits.instance[key2]['data'])/2
            n_wav=self.spec[onum]['WAVELENGTH'].shape[1]
            self.spec[onum]['TIME']=np.tile(data.extra_fits.Time.data,(n_wav,1)).T
        ...
        # integration time
        self.time_int = np.mean(np.diff(data.extra_fits.Time.data))
        # can now delete data (no longer needed)
        del data
