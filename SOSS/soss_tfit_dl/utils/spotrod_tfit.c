...
/* Calculate orbital elements eta and xi.
  ...
  if (e == 0)
    ...
    Emid = M_PI - asin(k*e / sqrt(k*k + h*h * (1-e*e))) - atan2(k, - h * sqrt(1-k*k-h*h));
...
double *quad_limb_darkening(double* r, int n_r, double q1, double q2){
  double *ld;
  double u1,u2,oneminusmu;
  int i;
  
  u1 = 2.*sqrt(q1)*q2;
  u2 = sqrt(q1)*(1.-2.*q2);
  
  ld = malloc(n_r * sizeof(double));
  for (i = 0; i < n_r; ++i)
    {
      oneminusmu = 1.0 - sqrt(1.0 - r[i]*r[i]);
      ld[i]=1.-u1*oneminusmu - u2*oneminusmu*oneminusmu;
    }

  return ld;
}

void transitmodel(double *t,int n_t, double q1, double q2, double *spotx, double *spoty, double *spotrad, double *spotcont,
                     int n_spot, int n_r,
                     double t0,double period, double b, double sma, double rprs, double ec, double es,
                     double *tmodel){
  double *r, *f, *eta, *xi, *deltaT;
  double *planetx, *planety, *z, *planetangle;
  //, *planetangle1;
  int i, j;
  ...
  for (i = 0; i < n_r; ++i)
    {
      r[i]=(double) 1.0/(2*n_r)+(i/(n_r-1.))*(1.-1./n_r);
    }

  f=quad_limb_darkening(r,n_r,q1,q2);
  for (i = 0; i < n_r; ++i)
    {
      f[i]=2*f[i]/n_r;
    }
  ...
  eta = malloc(n_t * sizeof(double));
  xi = malloc(n_t * sizeof(double));
  deltaT = malloc(n_t * sizeof(double));
  for (i = 0; i < n_t; ++i)
    {
      deltaT[i]=t[i]-t0;
    }

  elements(deltaT, period, sma, ec, es, n_t, eta, xi);

  /*Calculate planet position and angle*/
  planetx = malloc(n_t * sizeof(double));
  planety = malloc(n_t * sizeof(double));
  z = malloc(n_t * sizeof(double));
  //planetangle needs to be initialized to 0
  planetangle = calloc(n_t*n_r, sizeof(double));

  int aa, bb;
  double zsqmpsq;
  double psquared = rprs*rprs;

  for (i = 0; i < n_t; ++i)
    {
      planetx[i] = -xi[i];
      planety[i] = b*eta[i]/sma;
      z[i] = sqrt(planetx[i]*planetx[i] + planety[i]*planety[i]);

      /*the code of circleangle() copied and adapted here, below*/
      zsqmpsq = z[i]*z[i]-psquared;

      if (rprs > z[i]) {
        // Planet covers center of star.
        aa = mybsearch(r, rprs-z[i], n_r);
        bb = mybsearch(r, rprs+z[i], n_r);
        for(j=0; j<aa; j++) {
          planetangle[i*n_r+j] = M_PI;
        }
        for(; j<bb; j++) {
          planetangle[i*n_r+j] = acos((r[j]*r[j]+zsqmpsq)/(2*z[i]*r[j]));
        }
        /* not needed now that planetangle is initialized to zero
        for(; j<n_r; j++) {
          planetangle[i*n_r+j] = 0.0;
        }*/
      } else {
        // Planet does not cover center of star.
        aa = mybsearch(r, z[i]-rprs, n_r);
        bb = mybsearch(r, z[i]+rprs, n_r);
        /* not needed now that planetangle is initialized to zero
        for(j=0; j<aa; j++) {
          planetangle[i*n_r+j] = 0.0;
        }*/
      ...
      /*the code of circleangle() copied and adapted here, above*/
    }

  /* Calculate lightcurve. */
  integratetransit(n_t, n_r, n_spot, planetx, planety, z, rprs, r, f,
                   spotx, spoty, spotrad, spotcont, planetangle, tmodel);
...
double period, t0, sma, b, e, omega, rprs;
double q1, q2,spotx[1],spoty[1],spotrad[1],spotcont[1];
int n_t,n_spot,n_r;
double *t, *tmodel;
...
for (i = 0; i < n_t; ++i)
  {
    t[i]=(double) i/(n_t-1);
    //t[i]=t[i]*0.3-0.15+0.5;
  }

n_r=1000;
tmodel = malloc(n_t * sizeof(double));

clock_t tic = clock();

transitmodel(t, n_t, q1, q2, spotx, spoty, spotrad, spotcont, n_spot, n_r, t0, period, b, sma, rprs, e*cos(omega), e*sin(omega), tmodel);
