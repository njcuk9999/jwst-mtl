...
def setup_gibbs():
    ...
    rsuf = np.array(
        [0.003, 3.0e-4, 3.0e-4, 3.0e-4, 3.0e-4, 0.001, 0.1, 1.0e-6, 1.0e-6, 1.0e-8, 0.01, 1.0e-6, 0.001, 0.001, \
         0.1, 0.1, 0.1, 0.1, 0.5, 1.0, 1.0])
    rmin = np.array(
        [1.0e-5, -10.0, -10.0, 0.0000, 0.0000, 0.000, -1.0e6, -1.0, -1.0e+5, 1.0e-6, 0.0, 0.0, -1.000, -1.000, \
...
def transitmodel_new(sol, time, itime=-1, ntt=0, tobs=0, omc=0, dtype=0, nintg=1):
...
#         dtypein = ones(len(time), dtype="int32") * int(dtype)  # contains data type, 0-photometry,1=RV data
    ...
    tmodel = empty_like(time)  # contains the transit model
    tfit5.transitmodel(nplanet, sol, time, itime, ntt, tobs, omc, tmodel, dtype, nintg)
    return tmodel;

def transitmodel(sol, time, itime=-1, ntt=0, tobs=0, omc=0, dtype=0, nintg=1):
    "read in transitmodel solution"
    nplanet = int((len(sol) - 8) / 10)  # number of planets
