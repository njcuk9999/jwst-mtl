...
def loic(ra, dec, sep, pa):
	theta = pa + 90 * uu.deg
	ra2 = ra1 + sep * np.cos(theta) / np.cos(dec)
	dec2 = dec1 + sep * np.sin(theta)
...
coord2b = loic(ra1, dec1, sep, 0*uu.deg)
coord3b = loic(ra1, dec1, sep, 90*uu.deg) 
coord4b = loic(ra1, dec1, sep, 180*uu.deg) 
coord5b = loic(ra1, dec1, sep, -90*uu.deg) 
